...
# =====================================================================
# director-wfs-custom-app.sh
#
# Fork of director-wfs.sh with the TESK-specific install replaced by a
# generic, parameterized ArgoCD Application. All cluster bootstrap steps
# (docker/kubectl/kind/helm, kind cluster, Cilium/ingress-nginx/CoreDNS,
# Argo CD itself) are unchanged. Search this file for
# "CUSTOMIZE YOUR ARGOCD APP HERE" for the block to edit.
# =====================================================================
...
# -----------------------------
# cert-manager CA secret (generic - kept from director-wfs.sh)
# Many Helm charts that ship their own cert-manager ClusterIssuer expect a
# pre-existing CA keypair secret to sign from. This is not TESK-specific;
# keep it if your chart's ClusterIssuer expects a secret named "ca-key-pair"
# in the cert-manager namespace, or delete this block if it doesn't.
# -----------------------------
...
# -----------------------------
# Wait for argocd/cilium/ingress-nginx to be ready (generic - kept from
# director-wfs.sh)
# -----------------------------
...
# =====================================================================
# >>>>>>>>>>>>>>>>>>>  CUSTOMIZE YOUR ARGOCD APP HERE  <<<<<<<<<<<<<<<<<<
# =====================================================================
# Everything above this point is generic cluster bootstrap (docker/kubectl/
# kind/helm, the kind cluster, Cilium/ingress-nginx/CoreDNS, and Argo CD
# itself) and is unchanged from director-wfs.sh.
#
# Everything below replaces the TESK-specific install with a *template* for
# whatever ArgoCD Application you want deployed instead. Fill in the values
# below, and edit CUSTOM_APP_VALUES_YAML further down to match your chart's
# values schema.
# =====================================================================

# --- App identity -----------------------------------------------------
CUSTOM_APP_NAME="${CUSTOM_APP_NAME:-my-app}"                 # ArgoCD Application name
CUSTOM_APP_NAMESPACE="${CUSTOM_APP_NAMESPACE:-my-app}"       # k8s namespace the app is deployed into
CUSTOM_APP_PROJECT="${CUSTOM_APP_PROJECT:-my-app}"           # ArgoCD AppProject name (created below)

# --- Source: Helm chart -------------------------------------------------
# If your chart lives in a Helm OCI/HTTP repo (like TESK's harbor repo did):
CUSTOM_APP_REPO_URL="${CUSTOM_APP_REPO_URL:-}"               # e.g. harbor.example.org/my-org/chart  OR  https://charts.example.org
CUSTOM_APP_CHART="${CUSTOM_APP_CHART:-}"                     # e.g. my-chart
CUSTOM_APP_TARGET_REVISION="${CUSTOM_APP_TARGET_REVISION:-}" # chart version, e.g. "1.0.0"

# If instead your app is defined by plain manifests/kustomize/git-hosted helm
# chart, set CUSTOM_APP_SOURCE_TYPE=git and fill these instead of the chart
# fields above:
CUSTOM_APP_SOURCE_TYPE="${CUSTOM_APP_SOURCE_TYPE:-helm}"     # "helm" (OCI/HTTP chart repo) or "git" (repoURL+path)
CUSTOM_APP_GIT_PATH="${CUSTOM_APP_GIT_PATH:-}"               # path within CUSTOM_APP_REPO_URL when SOURCE_TYPE=git

# If your Helm repo needs registering with ArgoCD as a private/OCI repo
# credential (like files/argo/repo.yaml did for TESK's harbor repo), set:
CUSTOM_APP_REPO_SECRET_NAME="${CUSTOM_APP_REPO_SECRET_NAME:-}"   # leave empty to skip registering a Repository secret
CUSTOM_APP_REPO_OCI="${CUSTOM_APP_REPO_OCI:-true}"               # "true" if CUSTOM_APP_REPO_URL is an OCI helm repo
CUSTOM_APP_REPO_USERNAME="${CUSTOM_APP_REPO_USERNAME:-}"         # optional
CUSTOM_APP_REPO_PASSWORD="${CUSTOM_APP_REPO_PASSWORD:-}"         # optional

WAIT_APP_TIMEOUT_SECS="${WAIT_APP_TIMEOUT_SECS:-1800}"  # 30 minutes

# -----------------------------
# AppProject + namespace
# -----------------------------
ensure_namespace_for_custom_app() {
  ensure_namespace "$CUSTOM_APP_NAMESPACE"
}

apply_argo_project() {
  log "Applying ArgoCD AppProject '${CUSTOM_APP_PROJECT}'..."
  kubectl --context "kind-${KIND_CLUSTER_NAME}" apply -f - <<EOF
apiVersion: argoproj.io/v1alpha1
kind: AppProject
metadata:
  name: ${CUSTOM_APP_PROJECT}
  namespace: ${ARGOCD_NS}
  finalizers:
    - resources-finalizer.argocd.argoproj.io
spec:
  description: "${CUSTOM_APP_NAME} project"
  sourceRepos:
    - '*'
  sourceNamespaces:
    - '*'
...
apply_argo_repo_secret() {
  # OPTIONAL: only runs if CUSTOM_APP_REPO_SECRET_NAME is set. This registers
  # CUSTOM_APP_REPO_URL as a Helm repo ArgoCD can pull from (mirrors what
  # files/argo/repo.yaml did for TESK's harbor repo).
  [[ -n "$CUSTOM_APP_REPO_SECRET_NAME" ]] || return 0
  [[ -n "$CUSTOM_APP_REPO_URL" ]] || die "CUSTOM_APP_REPO_SECRET_NAME set but CUSTOM_APP_REPO_URL is empty"

  log "Registering ArgoCD repository secret '${CUSTOM_APP_REPO_SECRET_NAME}' for ${CUSTOM_APP_REPO_URL}..."
  kubectl --context "kind-${KIND_CLUSTER_NAME}" -n "$ARGOCD_NS" apply -f - <<EOF
apiVersion: v1
kind: Secret
metadata:
  name: ${CUSTOM_APP_REPO_SECRET_NAME}
  namespace: ${ARGOCD_NS}
  labels:
    argocd.argoproj.io/secret-type: repository
stringData:
  enableOCI: "${CUSTOM_APP_REPO_OCI}"
  name: "${CUSTOM_APP_REPO_SECRET_NAME}"
  type: helm
  url: ${CUSTOM_APP_REPO_URL}
$( [[ -n "$CUSTOM_APP_REPO_USERNAME" ]] && echo "  username: ${CUSTOM_APP_REPO_USERNAME}" )
$( [[ -n "$CUSTOM_APP_REPO_PASSWORD" ]] && echo "  password: ${CUSTOM_APP_REPO_PASSWORD}" )
EOF
}
...
apply_argo_project_and_repo() {
  ...
  apply_argo_project
  apply_argo_repo_secret
  log "Applied AppProject (and repo secret, if configured)."
}
...
# =====================================================================
# >>>>>>>>>>>>>>>>>  YOUR APPLICATION + ITS HELM VALUES  <<<<<<<<<<<<<<<<
# =====================================================================
...
# --- START: replace with your chart's values ---
ingress:
  enabled: true
  host: "${DNS_NAME}"
...
template_and_apply_custom_app() {
  local values_yaml
  values_yaml="$(build_custom_app_values_yaml)"

  log "Templating ArgoCD Application '${CUSTOM_APP_NAME}':"
  log "  namespace: $CUSTOM_APP_NAMESPACE"
  log "  project:   $CUSTOM_APP_PROJECT"
  log "  source:    $CUSTOM_APP_SOURCE_TYPE"

  local source_block
  if [[ "$CUSTOM_APP_SOURCE_TYPE" == "git" ]]; then
    [[ -n "$CUSTOM_APP_REPO_URL" ]] || die "CUSTOM_APP_REPO_URL is required"
    [[ -n "$CUSTOM_APP_GIT_PATH" ]] || die "CUSTOM_APP_GIT_PATH is required when CUSTOM_APP_SOURCE_TYPE=git"
    source_block="$(cat <<EOF
    repoURL: "${CUSTOM_APP_REPO_URL}"
    path: "${CUSTOM_APP_GIT_PATH}"
    targetRevision: "${CUSTOM_APP_TARGET_REVISION:-HEAD}"
    helm:
      valuesObject:
$(printf '%s\n' "$values_yaml" | sed 's/^/        /')
EOF
    ...
    [[ -n "$CUSTOM_APP_REPO_URL" ]]        || die "CUSTOM_APP_REPO_URL is required"
    [[ -n "$CUSTOM_APP_CHART" ]]           || die "CUSTOM_APP_CHART is required"
    [[ -n "$CUSTOM_APP_TARGET_REVISION" ]] || die "CUSTOM_APP_TARGET_REVISION is required"
    source_block="$(cat <<EOF
    repoURL: "${CUSTOM_APP_REPO_URL}"
    chart: ${CUSTOM_APP_CHART}
    targetRevision: "${CUSTOM_APP_TARGET_REVISION}"
    helm:
      valuesObject:
$(printf '%s\n' "$values_yaml" | sed 's/^/        /')
EOF
...
metadata:
  ...
  - resources-finalizer.argocd.argoproj.io
spec:
  project: ${CUSTOM_APP_PROJECT}
  destination:
    namespace: ${CUSTOM_APP_NAMESPACE}
    server: https://kubernetes.default.svc
  syncPolicy:
    automated:
      enabled: true
...
EOF
  ...
  log "Applied ArgoCD Application '${CUSTOM_APP_NAME}'."
}

template_and_apply_custom_app

# -----------------------------
# Wait for the app to sync + go healthy
# -----------------------------
get_app_sync_status() {
  kubectl --context "kind-${KIND_CLUSTER_NAME}" -n "$ARGOCD_NS" \
    get application "$CUSTOM_APP_NAME" -o jsonpath='{.status.sync.status}' 2>/dev/null || true
}
get_app_health_status() {
  kubectl --context "kind-${KIND_CLUSTER_NAME}" -n "$ARGOCD_NS" \
    get application "$CUSTOM_APP_NAME" -o jsonpath='{.status.health.status}' 2>/dev/null || true
}

wait_for_argocd_application_synced_healthy() {
  log "Waiting up to ${WAIT_APP_TIMEOUT_SECS}s for Argo Application '${CUSTOM_APP_NAME}' to be Synced + Healthy..."
  ...
  kubectl --context "kind-${KIND_CLUSTER_NAME}" -n "$ARGOCD_NS" get application "$CUSTOM_APP_NAME" -o yaml || true

  warn "Argo Application conditions:"
  kubectl --context "kind-${KIND_CLUSTER_NAME}" -n "$ARGOCD_NS" get application "$CUSTOM_APP_NAME" \
...
print_final_outputs() {
  ...
  echo
  echo "========================================"
  echo "${CUSTOM_APP_NAME} deployed successfully"
  echo "========================================"
  ...
  echo
  echo "ArgoCD Application:   ${CUSTOM_APP_NAME} (namespace: ${CUSTOM_APP_NAMESPACE})"
  echo "========================================"
  echo
}
